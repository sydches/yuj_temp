...
class Session:
    ...
    def _write_trace(self, entry: dict) -> None:
        ...
        target is configured (with_yuj arm). Uses the in-memory trace
...
#
# The harness does NOT rewrite benchmark-specific paths (e.g. /testbed,
# /tmp/work) that appear in pretest output. Those are benchmark container
# mount conventions — naming them in harness code would encode per-
# benchmark knowledge in the wrong layer. The benchmark's prepare script
# is responsible for sanitizing its own output before returning. See
# scripts/prepare.sh: the generated pretest.sh pipes its output through
# sed before exiting. Harness sees already-clean text.
...
def run_pretest(repo_dir: Path, *, pretest_script: Path | None = None, pretest_timeout: int,
    ...
    The harness knows nothing about which benchmark it is running. The
    benchmark-specific prepare script (``scripts/prepare.sh`` for FB) is
    responsible for materializing an executable shell command that produces
    the failing-test verdict for this task in whatever environment the
    benchmark needs (Docker container, conda env, bare interpreter, etc.),
    AND for sanitizing any benchmark-specific paths before returning. The
    ...
    is ``repo_dir.parent.parent`` (the layout prepare.sh produces:
    ``run_dir/repos/<iid>/``) and ``<iid>`` is the repo dir's name. The
    script lives OUTSIDE the agent's cwd so a plain ``find .`` in the
...
def solve_task(
    ...
    """
    prompt_file = repo_dir / "prompt.txt"
    ...
    if task_prompt is None:
        if not prompt_file.exists():
            log.error("No prompt.txt in %s", repo_dir)
            return False
    ...
    trace_path = repo_dir / ".trace.jsonl"
    task_description = ""

    # Mechanical state.json writer: active iff .solver/state.json was seeded
    # at prepare time (with_yuj arm only). wo_yuj runs have no .solver/ dir,
    # so state_path stays None and the state writer is a no-op.
    state_json_path = repo_dir / ".solver" / "state.json"
    ...
    # observability. Written OUTSIDE repo_dir (the agent's sandbox cwd)
    # so its mere existence in `ls -la` doesn't change with wall-clock
    # mtime across runs — under temp=0 a single timestamp char flips
    # the model's path. One file per task at run_dir/savings/<task>.jsonl.
    from .savings import open_ledger, close_ledger, get_ledger
    _savings_run_dir = repo_dir.parent.parent  # run_dir/repos/<iid>/
    _savings_dir = _savings_run_dir / "savings"
    ...
    # Written OUTSIDE repo_dir (the agent's sandbox cwd) so the model's
    # grep -r never finds reflections of its own prior HTTP exchanges.
    # Prior location was repo_dir/transcript.log; moved to run_dir level
    # for determinism — chatcmpl UUIDs in transcript.log varied per run
    # and leaked into grep results, flipping temp=0 deterministic paths.
    if hasattr(client, "set_transcript"):
        run_dir = repo_dir.parent.parent  # run_dir/repos/<iid>/
        transcript_dir = run_dir / "transcripts"
        transcript_dir.mkdir(parents=True, exist_ok=True)
        client.set_transcript(transcript_dir / f"{repo_dir.name}.log")
    ...
    with open(trace_path, "a") as trace_file:
        for session_num in range(1, cfg.max_sessions + 1):
            ...
            if session_num > 1 and _pretest_is_green(pretest_block):
                ...
                write_checkpoint(repo_dir, cfg.model, "completed")
                success = True
                sessions_used = session_num - 1
                break

            # Cross-run determinism: re-normalize file timestamps right
            # before the agent's first turn. prepare.sh did this already,
            # but pretest just ran in Docker and bumped directory mtimes
            # via __pycache__ create/delete cycles. Without this, the
            # very first `ls -la` shows different timestamps per run and
            # deterministically flips the model's path under temp=0.
            # Done only on session 1 — later sessions reflect the agent's
            # own state mutations and must not be touched.
            if session_num == 1:
                _normalize_repo_timestamps(repo_dir)

            if session_num == 1:
                initial = task_prompt
                if cfg.prompt_addendum:
            ...
            log.info("[session %d/%d] %s", session_num, cfg.max_sessions, repo_dir.name)
            ...
            if context_class is not None:
                ...
                ctx = None
            session = Session(
                cfg, client, system_prompt, initial, str(repo_dir),
            ...
            if result.done:
                _auto_commit(repo_dir, session_num, result.finish_reason)
                write_checkpoint(repo_dir, cfg.model, "completed")
                success = True
                break

            if result.finish_reason == "error":
                write_checkpoint(repo_dir, cfg.model, "error")
                break
            ...
            log.warning("Max sessions (%d) exhausted for %s", cfg.max_sessions, repo_dir.name)
            write_checkpoint(repo_dir, cfg.model, "error")
    ...
    write_run_metrics(repo_dir, metrics, provenance)
